...
use super::super::oci::{LinuxDevice, LinuxDeviceType, Mount, Spec};
use super::cgroups;
use super::nix_ext::*;
...
pub fn init_rootfs(
    ...
    chdir(rootfs).map_err(|e| Error::IOError(format!("io error is {:?}", e)))?;

    default_symlinks()?;
    create_devices(&linux.devices, bind_devices)?;
    ensure_ptmx()?;

    chdir(&olddir).map_err(|e| Error::IOError(format!("io error is {:?}", e)))?;
