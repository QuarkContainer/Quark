...
impl VMSpace {
    ...
    pub fn ReadContent(filenameAddr: u64, filenameLen: usize, addr: u64, len: usize) -> i64 {
        ...
        let filenameSlice =
            unsafe { slice::from_raw_parts(filenameAddr as *const u8, filenameLen) };

        let filename = str::from_utf8(filenameSlice).unwrap();
        let mut f = match File::open(filename) {
            Err(e) => {
                error!(
                    "ReadContent for file {} open fail with error {:?}",
                    filename, e
                );
                return -2;
            }
        ...
        let count = match f.read(buf) {
            Ok(n) => n,
            Err(e) => {
                error!(
                    "ReadContent for file {} read fail with error {:?}",
                    filename, e
                );
                return -2;
            }
