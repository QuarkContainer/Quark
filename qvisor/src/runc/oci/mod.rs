...
const VERSION_PATCH: i32 = 1;

// VersionDev indicates development branch. Releases will be empty string.
const VERSION_DEV: &str = "-dev";
...
impl Default for LinuxCapabilityType {
    fn default() -> LinuxCapabilityType {
...
fn cap_from_array<D>(a: &[serde_json::Value]) -> Result<Vec<LinuxCapabilityType>, D::Error>
where
...
fn deserialize_capabilities<D>(de: D) -> Result<Option<LinuxCapabilities>, D::Error>
where
    D: serde::Deserializer,
{
...
impl Default for LinuxSeccompOperator {
    fn default() -> LinuxSeccompOperator {
        LinuxSeccompOperator::SCMP_CMP_NE
    }
}
#[derive(Serialize, Deserialize, Debug, Default)]
...
impl Spec {
    ...
    pub fn from_string(str: &str) -> Result<Self, serialize::SerializeError> {
        serialize::from_string(str)
    }

    pub fn Copy(&self) -> Self {
        let str = self.to_string().unwrap();
        return Self::from_string(&str).unwrap();
    }
}
