...
#[derive(Default)]
pub struct MemoryManagerInternal2 {
    pub uid: UniqueID,
    pub inited: bool,
...
#[derive(Clone)]
pub struct MemoryManager(Arc<MemoryManagerInternal2>);

impl Deref for MemoryManager {
    type Target = Arc<MemoryManagerInternal2>;

    fn deref(&self) -> &Arc<MemoryManagerInternal2> {
        &self.0
    }
}
...
pub struct MemoryManagerWeak {
    pub uid: UniqueID,
    pub data: Weak<MemoryManagerInternal2>,
}
...
impl MemoryManager {
    pub fn Init() -> Self {
        ...
        let internal = MemoryManagerInternal2 {
            uid: NewUID(),
            inited: true,
    ...
    pub fn RemoveVMAsLocked(&self, ar: &Range) -> Result<()> {
        ...
        while vseg.Ok() && vseg.Range().Start() < ar.End() {
            ...
            if vma.mappable.is_some() {
                ...
                mappable.RemoveMapping2(self, &r, vma.offset, vma.CanWriteMappableLocked())?;
            }
    ...
    pub fn Fork(&self) -> Result<Self> {
        ...
        let layout = *self.layout.lock();
        let mmIntern2 = MemoryManagerInternal2 {
            uid: NewUID(),
            inited: true,
            layout: Mutex::new(layout),
            ..Default::default()
        ...
        let mm2 = MemoryManager(Arc::new(mmIntern2));
        {
            ...
            while srcvseg.Ok() {
                ...
                if vma.mappable.is_some() {
                    ...
                    match mappable.AddMapping2(&mm2, &vmaAR, vma.offset, vma.CanWriteMappableLocked()) {
                        Err(e) => {
...
impl MemoryManagerInternal {
    ...
    //Remove virtual memory to the phy mem mapping
    pub fn RemoveVMAsLocked(&mut self, mm: &MemoryManager1, ar: &Range) -> Result<()> {
...
#[derive(Clone)]
pub struct MemoryManagerWeak1 {
    pub uid: UniqueID,
    pub data: Weak<RwLock<MemoryManagerInternal>>,
}

impl MemoryManagerWeak1 {
    pub fn ID(&self) -> UniqueID {
        return self.uid;
    }

    pub fn Upgrade(&self) -> MemoryManager1 {
        return MemoryManager1 {
            uid: self.uid,
...
#[derive(Clone)]
pub struct MemoryManager1 {
    pub uid: UniqueID,
    pub data: Arc<RwLock<MemoryManagerInternal>>,
}

impl Deref for MemoryManager1 {
...
impl MemoryManager1 {
    ...
    pub fn Downgrade(&self) -> MemoryManagerWeak1 {
        return MemoryManagerWeak1 {
            uid: self.uid,
    ...
    pub fn InstallPageWithAddr(&self, task: &Task, pageAddr: u64) -> Result<()> {
        let (vma, range) = match self.GetVmaAndRange(pageAddr) {
            None => return Err(Error::SysError(SysErr::EFAULT)),
            Some(data) => data
        };
