...
impl MemoryManager {
    ...
    pub fn FindAvailableLocked(&self, length: u64, opts: &mut FindAvailableOpts) -> Result<u64> {
        ...
        error!("FindAvailableLocked 1");

        defer!(error!("finish FindAvailableLocked"));
        ...
        error!("FindAvailableLocked 4");
        if opts.Map32Bit {
            error!("FindAvailableLocked 4.1");
    ...
    pub fn CreateVMAlocked(&self, _task: &Task, opts: &MMapOpts) -> Result<(AreaSeg<VMA>, Range)> {
        ...
        if opts.Mappable.is_some() {
            ...
            mappable.AddMapping2(self, &ar, opts.Offset, !opts.Private && opts.MaxPerms.Write())?;
        }
...
impl MemoryManagerInternal {
    ...
    pub fn CreateVMAlocked(&mut self, _task: &Task, opts: &MMapOpts, mm: &MemoryManager1) -> Result<(AreaSeg<VMA>, Range)> {
        if opts.MaxPerms != opts.MaxPerms.Effective() {
            panic!("Non-effective MaxPerms {:?} cannot be enforced", opts.MaxPerms);
        }
