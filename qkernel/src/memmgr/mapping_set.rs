...
#[derive(Clone)]
pub struct MappingOfRange2 {
    pub MappingSpace: MemoryManagerWeak,
    pub AddrRange: Range,
    pub Writeable: bool,
}

impl Ord for MappingOfRange2 {
...
impl Eq for MappingOfRange2 {}

impl PartialEq for MappingOfRange2 {
...
impl PartialOrd for MappingOfRange2 {
...
pub struct MappingsOfRange2(pub Arc<Mutex<BTreeSet<MappingOfRange2>>>);

impl Deref for MappingsOfRange2 {
    type Target = Arc<Mutex<BTreeSet<MappingOfRange2>>>;

    fn deref(&self) -> &Arc<Mutex<BTreeSet<MappingOfRange2>>> {
        &self.0
    }
}

impl MappingsOfRange2 {
    pub fn New() -> Self {
        return Self(Arc::new(Mutex::new(BTreeSet::new())));
    }
}

impl AreaValue for MappingsOfRange2 {
...
fn SubsetMapping2(wholeRange: &Range, subsetRange: &Range, ms: &MemoryManagerWeak, addr: u64, writeable: bool) -> MappingOfRange2 {
...
impl AreaSet<MappingsOfRange2> {
    ...
    pub fn AddMapping(&mut self, ms: &MemoryManager, ar: &Range, offset: u64, writeable: bool) -> Vec<Range> {
        ...
        loop {
            if seg.Ok() && seg.Range().Start() < mr.End() {
                ...
                let val = seg.Value();
                val.lock().insert(SubsetMapping2(&mr, &seg.Range(), &ms.Downgrade(), ar.Start(), writeable));
                ...
                seg = self.Insert(&gap, &gapMR, MappingsOfRange2::New());
                gap = AreaGap::default();
            } else {
                return mapped
            }
    ...
    pub fn RemoveMapping(&mut self, ms: &MemoryManager, ar: &Range, offset: u64, writeable: bool) -> Vec<Range> {
        ...
        while seg.Ok() && seg.Range().Start() < mr.End() {
            ...
            mappings.lock().remove(&SubsetMapping2(&mr, &seg.Range(), &ms.Downgrade(), ar.Start(), writeable));
    ...
    pub fn InvalidateRanges(&mut self, _task: &Task, mr: &Range, _invalidatePrivate: bool) -> Vec<MappingOfRange2> {
        let mut ranges = Vec::new();
        let mut seg = self.LowerBoundSeg(mr.Start());
        while seg.Ok() && seg.Range().Start() < mr.End() {
            let segMR = seg.Range();
            for m in seg.Value().lock().iter() {
                let region = SubsetMapping2(&segMR, &segMR.Intersect(mr), &m.MappingSpace, m.AddrRange.Start(), m.Writeable);
                ranges.push(region);
            }
...
pub struct MappingOfRange {
    pub MappingSpace: MemoryManagerWeak1,
    pub AddrRange: Range,
    pub Writeable: bool,
}
...
// 0x2000). Then for subsetRange = [0x1000, 0x2000), subsetMapping returns a
// MappingOfRange for which AddrRange = [0x5000, 0x6000).
fn SubsetMapping(wholeRange: &Range, subsetRange: &Range, ms: &MemoryManagerWeak1, addr: u64, writeable: bool) -> MappingOfRange {
    if !wholeRange.IsSupersetOf(&subsetRange) {
        panic!("{:?} is not a superset of {:?}", wholeRange, subsetRange);
    }
...
impl AreaSet<MappingsOfRange> {
    ...
    //
    // Preconditions: As for Mappable.AddMapping.
    pub fn AddMapping(&mut self, ms: &MemoryManager1, ar: &Range, offset: u64, writeable: bool) -> Vec<Range> {
        let mr = Range::New(offset, ar.Len());
        let mut mapped = Vec::new();
    ...
    //
    // Preconditions: As for Mappable.RemoveMapping.
    pub fn RemoveMapping(&mut self, ms: &MemoryManager1, ar: &Range, offset: u64, writeable: bool) -> Vec<Range> {
        let mr = Range::New(offset, ar.Len());
        let mut unmapped = Vec::new();
