...
pub fn GetSockOptSocket(task: &Task,
                        s: &FileOperations,
                        ep: &BoundEndpoint,
                        family: i32,
                        skType: i32,
                        name: i32,
                        outlen: usize) -> Result<SockOptResult> {
    match name as u64 {
        LibcConst::SO_TYPE => {
        ...
        LibcConst::SO_ERROR => {
            ...
            match ep.GetSockOpt(&mut SockOpt::ErrorOption) {
                ...
                Err(Error::SysError(syserr)) => {
                    return Ok(SockOptResult::I32(syserr))
                }
                Err(e) => {
...
pub fn GetSockOpt(task: &Task,
    ...
    match level {
        SOL_SOCKET => {
            return GetSockOptSocket(task, s, ep, family, skType, name, outlen)
        }
        SOL_TCP |
        SOL_IPV6 |
        SOL_IP |
...
pub fn SetSockOptSocket(_task: &Task,
                        s: &FileOperations,
                        ep: &BoundEndpoint,
                        name: i32,
                        optVal: &[u8]) -> Result<()> {
    match name {
        SO_SNDBUF => {
            ...
            assert!(optVal.len() >= 4);
            let val = unsafe {
                *(&optVal[0] as * const _ as u64 as * const i32)
            };
...
pub fn SetSockOpt(task: &Task,
    ...
    match level {
        SOL_SOCKET => {
            return SetSockOptSocket(task, s, ep, name, optVal)
        }
        SOL_TCP |
        SOL_IPV6 |
        SOL_IP |
