...
//use crate::task::*;
//use crate::threadmgr::task_sched::*;
use crate::{MainRun, SHARESPACE};

// refer to code : qkernel/src/interrupt/x86_64/mod.rs errorcode for x86:
// autogenerated by HW for aarch64 we have ESR as equivalent. But we need to fit
// the semantics
//
// 1. figure out what these regs mean in x86 in the context of PF handling
//      rcx, cr2, rip, eflags, r11
//  |--> DONE List:
//      |->cr2 -> FAR_EL1
//  |-->Pending (Probably not needed)
//      |->rpi(/rcx) -> ?
//      |->eflags(/r11) -> ? 
// 2. map errorcode to ESR defs
//  |-->[RFC] DONE
// 3. determine whether a PF is "fromuser"
//  |-->[RFC] DONE
// 4. implement the from_bits (ESR)
//    or with other parsers anything that maps the x86 errorcode to this.
//  |-->[RFC] DONE
// 5. Pagetables - implement SwapInPage()...
// 6. GetVmaAndRangeLocked takes cr2, check that out
//
...
pub fn PageFaultHandler(ptRegs: &mut PtRegs, fault_address: u64,
    ...
    loop {
        ...
        if error_code.is_flag_set(PageFaultErrorFlags::FaultAccessFlag) {
            panic!("VM: PTE has not set AccessFlag.");
        }

        if vma.kernel == true {
            let k_map = currTask.mm.GetSnapshotLocked(currTask, false);
            info!("VM: It is k_map {}", &k_map);
            signal = Signal::SIGSEGV;
            break;
        }
