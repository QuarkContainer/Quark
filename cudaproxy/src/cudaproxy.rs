...
pub extern "C" fn cudaDeviceGetAttribute(
    ...
    println!(
        "Hijacked cudaDeviceGetAttribute(device: {}, cudaDeviceAttr: {:?}) ",
        device, attr
    );
...
pub extern "C" fn cudaDeviceGetCacheConfig(pCacheConfig: *mut cudaFuncCache) -> usize {
    ...
    println!(
        "cudaFuncCache struct address is: {:x}",
        pCacheConfig as usize
    );
...
pub extern "C" fn cudaDeviceGetLimit(pValue: *mut usize, limit: cudaLimit) -> usize {
    println!(
        "Hijacked cudaDeviceGetLimit(limit want to query: {:?}) ",
        limit
    );
...
pub extern "C" fn cudaDeviceGetSharedMemConfig(pConfig: *mut cudaSharedMemConfig) -> usize {
    unsafe {
        println!(
            "Hijacked cudaDeviceGetSharedMemConfig(cudaSharedMemConfig: {:?})",
            *pConfig as u32
        );
    };
...
pub extern "C" fn cudaDeviceReset() -> usize {
    ...
    return unsafe { syscall1(SYS_PROXY, ProxyCommand::CudaDeviceReset as usize) };
}
...
pub extern "C" fn cudaDeviceSetCacheConfig(cacheConfig: cudaFuncCache) -> usize {
    println!(
        "Hijacked cudaDeviceSetCacheConfig(cacheConfig setting: {:#?})",
        cacheConfig
    );
    return unsafe {
...
pub extern "C" fn cudaDeviceSynchronize() -> usize {
    ...
    return unsafe { syscall1(SYS_PROXY, ProxyCommand::CudaDeviceSynchronize as usize) };
}
...
pub extern "C" fn cudaGetDeviceCount(count: *mut c_int) -> usize {
    unsafe { println!("Hijacked cudaGetDeviceCount({})", *count) };
...
pub extern "C" fn __cudaUnregisterFatBinary(fatCubinHandle: u64) {
    println!(
        "Hijacked __cudaUnregisterFatBinary(fatCubinHandle = {:x})",
        fatCubinHandle
    );
...
pub extern "C" fn __cudaRegisterVar(
    ...
    println!(
        "Hijacked __cudaRegisterVar(fatCubinHandle:{:x}, hostVar:{:x}, 
...
pub extern "C" fn cudaStreamCreate(pStream: *mut cudaStream_t) -> usize {
    println!(
        "Hijacked cudaStreamCreate, stream address: {:x}",
        pStream as u64
    );
    unsafe {
...
pub extern "C" fn cudaStreamIsCapturing(
    ...
    unsafe {
        println!(
            "Hijacked cudaStreamIsCapturing,, captureStatus is :{}",
            (*pCaptureStatus) as u64
        );
    };
...
pub extern "C" fn cudaGetLastError() -> usize {
    ...
    return unsafe { syscall1(SYS_PROXY, ProxyCommand::CudaGetLastError as usize) };
}
