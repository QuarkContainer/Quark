...
impl RingeBufAllocator {
    ...
    pub fn FreeHeadTail(&self, data: &'static [AtomicU32]) {
        match self {
            Self::GuestPrivateHeapAllocator => return GuestPrivateHeapAllocator::FreeHeadTail(data),
    ...
    pub fn FreeWaitingRW(&self, data: &'static [AtomicBool]) {
        match self {
            Self::GuestPrivateHeapAllocator => return GuestPrivateHeapAllocator::FreeWaitingRW(data),
    ...
    pub fn AlllocBuf(&self, pageCount: usize) -> u64 {
        match self {
            Self::GuestPrivateHeapAllocator => return GuestPrivateHeapAllocator::AlllocBuf(pageCount),
            Self::GuestSharedHeapAllocator => return GuestSharedHeapAllocator::AlllocBuf(pageCount),
    ...
    pub fn FreeBuf(&self, addr: u64, size: usize) {
        match self {
            Self::GuestPrivateHeapAllocator => return GuestPrivateHeapAllocator::FreeBuf(addr, size),
...
impl GuestSharedHeapAllocator {
    ...
    pub fn FreeHeadTail(data: &'static [AtomicU32]) {
        ...
        let layout =
            Layout::from_size_align(8, 8).expect("GuestSharedHeapAllocator::FreeHeadTail can't free memory");
    ...
    pub fn FreeWaitingRW(data: &'static [AtomicBool]) {
        ...
        let addr = &data[0] as *const _ as u64;
        let layout =
            Layout::from_size_align(2, 8).expect("GuestSharedHeapAllocator::FreeWaitingRW can't free memory");
        unsafe { dealloc(addr as *mut u8, layout) };
    }
