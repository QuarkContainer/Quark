...
impl SocketInodeOps {
    pub fn New(task: &Task, ep: &BoundEndpoint, owner: &FileOwner, perms: &FilePermissions) -> Self {
        return Self {
            ep: ep.clone(),
            simpleAttributes: Arc::new(InodeSimpleAttributes::New(task, owner, perms, FSMagic::SOCKFS_MAGIC)),
...
impl InodeOperations for SocketInodeOps {
    ...
    fn Getxattr(&self, dir: &Inode, name: &str) -> Result<String> {
        return self.simpleExtendedAttribute.Getxattr(dir, name)
    }

    fn Setxattr(&self, dir: &mut Inode, name: &str, value: &str) -> Result<()> {
        return self.simpleExtendedAttribute.Setxattr(dir, name, value)
    }

    fn Listxattr(&self, dir: &Inode) -> Result<Vec<String>> {
        return self.simpleExtendedAttribute.Listxattr(dir)
    }
    ...
    fn SetPermissions(&self, task: &Task, _dir: &mut Inode, p: FilePermissions) -> bool {
        self.simpleAttributes.write().unstable.SetPermissions(task, &p);
        return true;
    }
    ...
    fn SetTimestamps(&self, task: &Task, _dir: &mut Inode, ts: &InterTimeSpec) -> Result<()> {
        self.simpleAttributes.write().unstable.SetTimestamps(task, ts);
        return Ok(())
    }
