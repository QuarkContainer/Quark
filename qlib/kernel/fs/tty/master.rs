...
pub fn NewMasterNode(task: &Task, d: &DirInodeOperations, owner: &FileOwner, p: &FilePermissions) -> Inode {
...
impl InodeOperations for MasterInodeOperations {
    ...
    fn GetFile(&self, _task: &Task, _dir: &Inode, _dirent: &Dirent, _flags: FileFlags) -> Result<File> {
...
impl FileOperations for MasterFileOperations {
    ...
    fn ReadAt(&self, task: &Task, _f: &File, dsts: &mut [IoVec], _offset: i64, _blocking: bool) -> Result<i64> {
        ...
        let cnt = self.t.ld.lock().OutputQueueRead(task, &mut buf[..size as usize])? as usize;
    ...
    fn WriteAt(&self, task: &Task, _f: &File, srcs: &[IoVec], _offset: i64, _blocking: bool) -> Result<i64> {
        ...
        let res = self.t.ld.lock().InputQueueWrite(task, &mut buf.buf[0..size as usize])?;
        return Ok(res)
    }
    ...
    fn Ioctl(&self, task: &Task, _f: &File, _fd: i32, request: u64, val: u64) -> Result<()> {
        ...
        match cmd {
            IoCtlCmd::FIONREAD => {
                return self.t.ld.lock().OutputQueueReadSize(task, val)
            }
            IoCtlCmd::TCGETS => {
                return self.t.ld.lock().GetTermios(task, val)
            }
            IoCtlCmd::TCSETS => {
