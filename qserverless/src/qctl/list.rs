...
impl ListCmd {
    ...
    pub async fn Run(&self) -> Result<()> {
        ...
        for p in packages {
            objs.push(PackageMeta {
                name: p.metadata.name.as_ref().unwrap().clone(),
                namespace: self.namespace.clone(),
                createTime: p.metadata.creation_timestamp.clone(),
            })
        }

        println!("packages is {:#?}", objs);
        return Ok(())
    }
}