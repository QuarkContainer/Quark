...
lazy_static! {
    ...
    pub static ref QUALIFIED_NAME_REGEXP : Regex = Regex::new(&("^".to_owned() + &QUALIFIED_NAME_FMT + "$")).unwrap();
}
...
pub fn IsQualifiedName(value: &str) -> Result<()> {
    ...
    let (_prefix, name) = match parts.len() {
        ...
        _ => {
            return Err(Error::CommonError(format!("'{}' is not a qualified name", value)));
        }
    };
    ...
    if name.len() > QUALIFIED_NAME_MAX_LENGTH {
        return Err(Error::CommonError("Name len must be less than 64".to_owned()));
    }
...
pub fn IsFullyQualifiedDomainName(name: &str) -> Result<()> {
    ...
    if parts.len() < 2 {
        return Err(Error::CommonError("should be a domain with at least two segments separated by dots".to_owned()));
    }
...
lazy_static! {
    pub static ref HTTP_PATH_REGEXP : Regex = Regex::new(&("^".to_owned() + &HTTP_PATH_FMT + "$")).unwrap();
}
...
pub fn IsDomainPrefixedPath(dpPath: &str) -> Result<()> {
    ...
    if segments.len() != 2 || segments[0].len() == 0 || segments[1].len() == 0 {
        return Err(Error::CommonError("must be a domain-prefixed path (such as \"acme.io/foo\"".to_owned()));
    }
...
pub fn IsValidLabelValue(val: &str) -> Result<()> {
    if val.len() > QUALIFIED_NAME_MAX_LENGTH {
        return Err(Error::CommonError(format!("must be no more than {} characters, actual {}", QUALIFIED_NAME_MAX_LENGTH, val.len())))
    }

    if !QUALIFIED_NAME_REGEXP.is_match(val) {
        return Err(Error::CommonError(format!("{} actual:{}", LABEL_VALUE_ERR_MSG, val)));
    }
    return Ok(())
}
...
lazy_static! {
    pub static ref DNS1123_LABEL_REGEXP : Regex = Regex::new(&("^".to_owned() + DNS1123_LABEL_FMT + "$")).unwrap();
}
...
pub fn IsDNS1123Label(value: &str) -> Result<()> {
    if value.len() > QUALIFIED_NAME_MAX_LENGTH {
        return Err(Error::CommonError("DNS1123Label len must be less than 64".to_owned()));
    }
    ...
    if !DNS1123_LABEL_REGEXP.is_match(value) {
        ...
        if DNS1123_SUBDOMAIN_REGEXP.is_match(value) {
            return Err(Error::CommonError("DNS1123Label must not contain dots".to_owned()));
        } else {
...
lazy_static! {
    pub static ref DNS1123_SUBDOMAIN_FMT: String = DNS1123_LABEL_FMT.to_string() + "(\\." + DNS1123_LABEL_FMT +  ")*";
    pub static ref DNS1123_SUBDOMAIN_REGEXP : Regex = Regex::new(&("^".to_owned() + &DNS1123_SUBDOMAIN_FMT + "$")).unwrap();
}
...
pub fn IsDNS1123Subdomain(value: &str) -> Result<()> {
    if value.len() > DNS1123_SUBDOMAIN_MAX_LENGTH {
        return Err(Error::CommonError("DNS1123Subdomain len must be less than 254".to_owned()));
    }
...
lazy_static! {
    pub static ref DNS1035_LABEL_REGEXP : Regex = Regex::new(&("^".to_owned() + DNS1035_LABEL_FMT + "$")).unwrap();
}
...
pub fn IsDNS1035Label(value: &str) -> Result<()> {
    if value.len() > DNS1035_LABEL_MAX_LENGTH {
        return Err(Error::CommonError("DNS1035 Label len must be less than 64".to_owned()));
    }
...
lazy_static! {
    pub static ref WILDCARD_DNS1123_SUBDOMAIN_FMT: String = "\\*\\.".to_string() + DNS1123_LABEL_FMT;
}
...
pub fn IsWildcardDNS1123Subdomain(value: &str) -> Result<()> { 
    let wildcardDNS1123SubdomainRegexp = Regex::new(&("^".to_owned() + &WILDCARD_DNS1123_SUBDOMAIN_FMT + "$")).unwrap();
    if value.len() > DNS1123_SUBDOMAIN_MAX_LENGTH {
        return Err(Error::CommonError("wildcard DNS1123 Subdomain len must be less than 254".to_owned()));
    }

    if !wildcardDNS1123SubdomainRegexp.is_match(value) {
        return Err(Error::CommonError(WILDCARD_DNS1123_SUBDOMAIN_ERR_MSG.to_string()));
    }
    return Ok(())
}
...
lazy_static! {
    pub static ref C_IDENTIFIER_REGEXP : Regex = Regex::new(&("^".to_owned() + C_IDENTIFIER_FMT + "$")).unwrap();
}
...
pub fn IsValidPortName(port: &str) -> Result<()> {
    if port.len() > 15 {
        return Err(Error::CommonError("port len must be less than 16".to_owned()));
    }

    if !PORT_NAME_CHARSET_REGEX.is_match(port) {
        return Err(Error::CommonError("must contain only alpha-numeric characters (a-z, 0-9), and hyphens (-)".to_owned()));
    }

    if !PORT_NAME_ONE_LETTER_REGEXP.is_match(port) {
        return Err(Error::CommonError("must contain at least one letter (a-z)".to_owned()));
    }

    if port.contains("--") {
        return Err(Error::CommonError("must not contain consecutive hyphens".to_owned()));
    }

    let chars: Vec<_> = port.chars().collect();
    if chars.len() > 0 && (chars[0] == '-' || chars[chars.len()-1] == '-') {
        return Err(Error::CommonError("must not begin or end with a hyphen".to_owned()));
    }
...
pub fn IsValidIPv4Address(value: &str) -> Result<()> {
    use std::net::IpAddr;
    match value.parse::<IpAddr>() {
        Err(_) => return Err(Error::CommonError("must be a valid IPv4 address".to_owned())),
        Ok(addr) => {
            if !addr.is_ipv4() {
                return Err(Error::CommonError("must be a valid IPv4 address".to_owned()));
            }
        }
    }
    return Ok(())
...
pub fn IsValidIPv6Address(value: &str) -> Result<()> {
    use std::net::IpAddr;
    match value.parse::<IpAddr>() {
        Err(_) => return Err(Error::CommonError("must be a valid IPv6 address".to_owned())),
        Ok(addr) => {
            if !addr.is_ipv6() {
                return Err(Error::CommonError("must be a valid IPv6 address".to_owned()));
            }
        }
    }
    return Ok(())
...
pub const PERCENT_ERR_MSG : &str = "a valid percent string must be a numeric string followed by an ending '%'";

lazy_static! {
    pub static ref PERCENT_REGEXP : Regex = Regex::new(&("^".to_owned() + PERCENT_FMT + "$")).unwrap();
}
...
pub const HTTP_HEADER_NAME_ERR_MSG : &str = "a valid HTTP header must consist of alphanumeric characters or '-'";

lazy_static! {
    pub static ref HTTP_HEADER_NAME_REGEXP : Regex = Regex::new(&("^".to_owned() + HTTP_HEADER_NAME_FMT + "$")).unwrap();
}
...
lazy_static! {
    pub static ref ENV_VAR_NAME_REGEXP : Regex = Regex::new(&("^".to_owned() + ENV_VAR_NAME_FMT + "$")).unwrap();
}
...
lazy_static! {
    pub static ref CONFIG_MAP_KEY_REGEXP : Regex = Regex::new(&("^".to_owned() + CONFIG_MAP_KEY_FMT + "$")).unwrap();
}

// IsConfigMapKey tests for a string that is a valid key for a ConfigMap or Secret
pub fn IsConfigMapKey(value: &str) -> Result<()> {
    if value.len() > 253 {
        return Err(Error::CommonError("port len must be less than 254".to_owned()));
    }
