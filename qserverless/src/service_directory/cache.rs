...
use tokio::sync::{mpsc::channel, mpsc::Receiver, mpsc::Sender, Notify};
...
impl Cache {
    pub fn Add(&self, obj: DataObject) {
        self.store.Add(&obj).ok();
    }

    pub fn Update(&self, obj: DataObject) {
        self.store.Update(&obj).ok();
    }

    pub fn Delete(&self, obj: DataObject) {
        self.store.Delete(&obj).ok();
    }
...
impl Cacher {
    ...
    pub async fn New(store: &EtcdStore, objType: &str, rev: i64) -> Result<Self> {
        ...
        let future = tokio::spawn(async move {
            ...
            loop {
                let (mut w, r) =
                    storeClone.Watch(&prefixClone, list.revision, SelectionPredicate::default())?;
                ...
                let list = storeClone
                    .List(&prefixClone, &ListOption::default())
                    .await?;
