...
impl NodeConfigurationInner {
    pub fn DefaultNodeConfiguration() -> Result<Self> {
        ...
        return Ok(Self {
            ...
            DatabaseURL: format!(
                "file:{}/db/{}?cache=shared&mode=rwc",
                DefaultRootPath, DefaultDBName
            ),
...
pub fn GetPodVolumeSubpathsDir(rootPath: &str, podUID: &str) -> String {
    return format!(
        "{}/{}",
        GetPodDir(rootPath, podUID),
        DefaultVolumeSubpathsDirName
    );
}
...
pub fn GetPodVolumeDir(rootPath: &str, podUID: &str, pluginName: &str, volumeName: &str) -> String {
    return format!(
        "{}/{}/{}",
        GetPodVolumesDir(rootPath, podUID),
        pluginName,
        volumeName
    );
}
