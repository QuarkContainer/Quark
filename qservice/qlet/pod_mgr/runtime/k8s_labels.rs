...
pub fn NewContainerAnnotations(
    container: &ContainerDef,
    pod: &Arc<RwLock<PodDef>>,
    restartCount: i32,
    overrideAnnotations: &BTreeMap<String, String>,
) -> HashMap<String, String> {
    let pod = &pod.read().unwrap();
    let mut annotations = HashMap::new();

    annotations.insert(
        CONTAINER_HASH_LABEL.to_string(),
        format!("{}", HashContainer(container)),
    );
    annotations.insert(
    ...
    if let Some(s) = &pod.deletion_grace_period_seconds {
        annotations.insert(
            POD_DELETION_GRACE_PERIOD_LABEL.to_string(),
            format!("{}", *s),
        );
    }
...
pub fn GetPodSandboxInfoFromLabels(labels: &BTreeMap<String, String>) -> LabeledPodSandboxInfo {
    ...
    for (k, v) in labels {
        if k != KUBERNETES_POD_NAME_LABEL
            && k != KUBERNETES_POD_NAMESPACE_LABEL
            && k != KUBERNETES_POD_UIDLABEL
        {
...
pub fn GetPodSandboxInfoFromAnnotations(
    annotations: BTreeMap<String, String>,
) -> AnnotatedPodSandboxInfo {
...
pub fn GetContainerInfoFromAnnotations(
    ...
    match GetUint64ValueFromLabel(annotations, CONTAINER_HASH_LABEL) {
        ...
        Err(_) => error!(
            "Unable to get label value from annotations label {} annotations {:?}",
            CONTAINER_HASH_LABEL, annotations
        ),
    }
    ...
    match GetIntValueFromLabel(annotations, CONTAINER_RESTART_COUNT_LABEL) {
        ...
        Err(_) => error!(
            "Unable to get label value from annotations label {} annotations {:?}",
            CONTAINER_RESTART_COUNT_LABEL, annotations
        ),
    }
    ...
    match GetInt64PointerFromLabel(annotations, POD_DELETION_GRACE_PERIOD_LABEL) {
        ...
        Err(_) => error!(
            "Unable to get label value from annotations label {} annotations {:?}",
            POD_DELETION_GRACE_PERIOD_LABEL, annotations
        ),
    }
    ...
    match GetJSONObjectFromLabel::<Vec<ContainerPort>>(annotations, CONTAINER_PORTS_LABEL) {
        ...
        Err(_) => error!(
            "Unable to get label value from annotations label {} annotations {:?}",
            CONTAINER_PORTS_LABEL, annotations
        ),
    }
...
pub fn GetIntValueFromLabel(labels: &BTreeMap<String, String>, label: &str) -> Result<i32> {
    match labels.get(label) {
        Some(v) => match v.parse::<i32>() {
            Ok(v) => return Ok(v),
            Err(_e) => Err(Error::CommonError(format!(
                "GetIntValueFromLabel val {} is not valid",
                v
            ))),
        },
        None => {
        ...
        Some(v) => match u64::from_str_radix(v, 16) {
            ...
            Err(_e) => {
                return Err(Error::CommonError(format!(
                    "GetUint64ValueFromLabel val {} is not valid",
                    v
                )))
            }
        },
...
pub fn GetInt64PointerFromLabel(labels: &BTreeMap<String, String>, label: &str) -> Result<i64> {
    match labels.get(label) {
        Some(v) => match i64::from_str_radix(v, 16) {
            Ok(v) => return Ok(v),
            Err(_e) => {
                return Err(Error::CommonError(format!(
                    "GetInt64PointerFromLabel val {} is not valid",
                    v
                )))
            }
        },
...
pub fn GetJSONObjectFromLabel<T: for<'a> Deserialize<'a>>(
    ...
    match labels.get(label) {
        ...
        None => {
            ...
            info!("Container doesn't have requested label, it may be an old or invalid container label {}", label);
            return Err(Error::CommonError(format!(
                "getJSONObjectFromLabel not exist label {}",
                label
            )));
        }
    }
