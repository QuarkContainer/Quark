...
use qshare::crictl::image_service_client::ImageServiceClient;
use qshare::crictl::runtime_service_client::RuntimeServiceClient;
use qshare::crictl::*;
...
impl CriClient {
    ...
    pub async fn RunPodSandbox(
        &self,
        config: Option<PodSandboxConfig>,
        runtimeHandler: &str,
    ) -> Result<String> {
    ...
    pub async fn ListPodSandbox(
        &self,
        filter: Option<PodSandboxFilter>,
    ) -> Result<Vec<PodSandbox>> {
        let request = tonic::Request::new(ListPodSandboxRequest { filter: filter });
    ...
    pub async fn ListContainers(&self, filter: Option<ContainerFilter>) -> Result<Vec<Container>> {
        let request = tonic::Request::new(ListContainersRequest { filter: filter });
    ...
    pub async fn UpdateContainerResources(
        &self,
        containerID: &str,
    ...
    pub async fn ExecSync(
        ...
        let response = self.RuntimeClient().exec_sync(request).await?.into_inner();
    ...
    pub async fn Exec(&self, request: ExecRequest) -> Result<ExecResponse> {
        let response = self.RuntimeClient().exec(request).await?.into_inner();
    ...
    pub async fn Attach(&self, request: AttachRequest) -> Result<AttachResponse> {
        let response = self.RuntimeClient().attach(request).await?.into_inner();
    ...
    pub async fn ListContainerStats(
        &self,
        filter: Option<ContainerStatsFilter>,
    ) -> Result<Vec<ContainerStats>> {
        let request = tonic::Request::new(ListContainerStatsRequest { filter: filter });
    ...
    pub async fn PodSandboxStats(&self, podSandboxID: &str) -> Result<PodSandboxStats> {
        ...
        if response.stats.is_none() {
            return Err(Error::CommonError(format!(
                "PodSandboxStats for {} get none stats",
                podSandboxID
            )));
        }
    ...
    pub async fn ListPodSandboxStats(
        &self,
        filter: Option<PodSandboxStatsFilter>,
    ) -> Result<Vec<PodSandboxStats>> {
        let request = tonic::Request::new(ListPodSandboxStatsRequest { filter: filter });
    ...
    pub async fn Version(&self, apiVersion: &str) -> Result<VersionResponse> {
        ...
        let response = self.RuntimeClient().version(request).await?.into_inner();
    ...
    pub async fn Status(&self, verbose: bool) -> Result<StatusResponse> {
        let request = tonic::Request::new(StatusRequest { verbose: verbose });

        let response = self.RuntimeClient().status(request).await?.into_inner();
    ...
    pub async fn ListImages(&self, filter: Option<ImageFilter>) -> Result<Vec<Image>> {
        let request = tonic::Request::new(ListImagesRequest { filter: filter });

        let response = self.ImageClient().list_images(request).await?.into_inner();

        return Ok(response.images);
    }
    // ImageStatus returns the status of the image.
    pub async fn ImageStatus(
        &self,
        ...
        let response = self.ImageClient().image_status(request).await?.into_inner();

        return Ok(response);
    }

    // PullImage pulls an image with the authentication config.
    pub async fn PullImage(
        &self,
        image: Option<ImageSpec>,
        ...
        let response = self.ImageClient().pull_image(request).await?.into_inner();
    ...
    pub async fn RemoveImage(&self, image: Option<ImageSpec>) -> Result<()> {
        let request = tonic::Request::new(RemoveImageRequest { image: image });

        let _response = self.ImageClient().remove_image(request).await?.into_inner();
