        ...
        D: serde::Deserializer<'de>,
    {
        ...
        if parts.next() != Some("#") {
            return Err(serde::de::Error::invalid_value(
                ...
                &"path like `#/definitions/$definitionName`",
            ));
        }
        ...
        let ref_path = parts.next().ok_or_else(|| {
...
impl RefPath {
    pub(crate) fn references_scope(&self, map_namespace: &impl crate::MapNamespace) -> bool {
        ...
        map_namespace
            .map_namespace(&path_parts[..(path_parts.len() - 1)])
            .is_none()
    }
}
        ...
        D: serde::Deserializer<'de>,
    {
        match ty {
            ...
            "object" => match additional_properties {
                Some(additional_properties) => Ok(Type::Object {
            ...
            "string" => {
                let format = match format {
                    ...
                    Some(format) => {
                        return Err(serde::de::Error::invalid_value(
                            serde::de::Unexpected::Str(format),
                            &"one of byte, date-time, int-or-string",
                        ))
                    }
                    None => None,
                };
            ...
            s => Err(serde::de::Error::invalid_value(
                ...
                &"one of array, boolean, integer, number, object, string",
            )),
        }
    }
}
