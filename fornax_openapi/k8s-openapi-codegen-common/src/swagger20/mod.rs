...
impl<'de> serde::Deserialize<'de> for Spec {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        ) -> Result<Operation, D::Error>
        where
            D: serde::Deserializer<'de>,
        {
            let responses: Result<_, _> = value
                .responses
                .into_iter()
                ...
                for parameter in &parameters {
                    if parameter.location == ParameterLocation::Body {
                        return Err(serde::de::Error::custom(format!(
                            "Operation {} has method GET but has a body parameter {}",
                            value.id, parameter.name
                        )));
                    }
                }
            }
        ...
        for (path, path_item) in result.paths {
            if let Some(delete) = path_item.delete {
                operations.push(parse_operation::<D>(
                    delete,
                    Method::Delete,
                    path.clone(),
