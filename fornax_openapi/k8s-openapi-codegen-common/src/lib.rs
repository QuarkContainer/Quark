        ...
        swagger20::SchemaKind::Properties(properties) => {
            let (template_properties, resource_metadata, metadata_ty) = {
                ...
                let resource_metadata =
                    match single_group_version_kind {
                        ...
                        Some((_, true, false)) => return Err(format!(
                            "{definition_path} has an apiVersion property but not a kind property"
                        )
                        .into()),
                        Some((_, false, true)) => return Err(format!(
    ...
    fn evaluate_trait_bound_inner<'a>(
        ...
        match &**kind {
            ...
            swagger20::SchemaKind::Ref(ref_path @ swagger20::RefPath { .. })
                if !ref_path.references_scope(map_namespace) =>
            {
                let trait_bound = if let Some(target) =
                    definitions.get(&swagger20::DefinitionPath(ref_path.path.clone()))
                {
                    let mut visited = visited.clone();
...
pub fn get_rust_ident(name: &str) -> std::borrow::Cow<'static, str> {
    ...
    for ((c, previous), next) in chars {
        if c.is_uppercase() {
            match (previous, next) {
                (Some(false), _) | (Some(true), Some(false)) => result.push('_'),
                _ => (),
            }
