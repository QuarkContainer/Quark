...
pub(crate) fn generate(
    ...
    let cfg = schema_feature.map_or_else(String::new, |schema_feature| {
        format!("#[cfg(feature = {schema_feature:?})]\n")
    });
...
fn gen_schema(
    ...
    writeln!(
        out,
        "{indent}{local}schemars::schema::Schema::Object({local}schemars::schema::SchemaObject {{"
    )?;

    if let Some(description) = &definition.description {
        writeln!(
            out,
            "{indent}    metadata: Some(Box::new({local}schemars::schema::Metadata {{"
        )?;
        writeln!(
            out,
            "{indent}        description: Some({description:?}.to_owned()),"
        )?;
    ...
    match &definition.kind {
        swagger20::SchemaKind::Properties(properties) => {
            ...
            writeln!(
                out,
                "{indent}    object: Some(Box::new({local}schemars::schema::ObjectValidation {{"
            )?;
            ...
            for (name, (schema, required)) in properties {
                ...
                match &schema.kind {
                    swagger20::SchemaKind::Properties(_) => {
                        unreachable!("unexpected nested properties")
                    }
                    ...
                    swagger20::SchemaKind::Ref(ref_path) => {
                        ...
                        if let Some(description) = &schema.description {
                            ...
                            writeln!(
                                props,
                                "{indent}                __gen.subschema_for::<{type_name}>(),"
                            )?;
                        }
        ...
        swagger20::SchemaKind::Ty(ty) => gen_type(out, ty, local, map_namespace, depth + 1)?,
...
fn gen_type_as_schema_object(
    ...
    if let swagger20::Type::CustomResourceSubresources(v) = ty {
        ...
        writeln!(
            out,
            "{indent}                __gen.subschema_for::<{json_schema_props_type_name}>(),"
        )?;
    } else {
