pub(crate) fn generate(
    ...
    let metav1 = {
        let namespace_parts = map_namespace
            .map_namespace(&["io", "k8s", "apimachinery", "pkg", "apis", "meta", "v1"])
    ...
    let type_generics_where = match operation_action {
        ...
        OperationAction::List => {
            format!(" where T: {local}serde::de::DeserializeOwned + {local}ListableResource")
        }
    };

    let operation_feature_attribute: std::borrow::Cow<'static, str> = operation_feature
        .map_or("".into(), |operation_feature| {
    ...
    match operation_action {
        OperationAction::Create => {
            ...
            variant_match_arms.push_str(&deserialize_single(
                &local, "CREATED", type_name, "Created",
            )?);
            variant_match_arms.push_str(&deserialize_single(
                &local, "ACCEPTED", type_name, "Accepted",
            )?);
        }
        ...
        OperationAction::Delete => {
            ...
            writeln!(
                variant_match_arms,
                "                    Err(err) => return Err({local}ResponseError::Json(err)),"
            )?;
            ...
            writeln!(
                variant_match_arms,
                "                    let result = result.map_err({local}ResponseError::Json)?;"
            )?;
            writeln!(
                variant_match_arms,
        ...
        OperationAction::Watch => {
            ...
            writeln!(
                variant_match_arms,
                "                    None => return Err({local}ResponseError::NeedMoreData),"
            )?;
            writeln!(variant_match_arms, "                }};")?;
            writeln!(
                variant_match_arms,
    ...
    writeln!(
        ...
        include_str!(concat!(
            ...
            "/templates/operation_response_common.rs"
        )),
        local = local,
        type_name = type_name,
...
fn deserialize_single(
    ...
    writeln!(
        result,
        "                let result = match {local}serde_json::from_slice(buf) {{"
    )?;
    ...
    writeln!(
        result,
        "                Ok(({type_name}::{variant_name}(result), buf.len()))"
    )?;
    writeln!(result, "            }},")?;
