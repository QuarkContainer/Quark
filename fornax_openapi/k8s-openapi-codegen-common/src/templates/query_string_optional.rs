pub(crate) fn generate(
    ...
    } in fields
    {
        ...
        match &**field_type_name {
            "Option<&'a str>" => writeln!(
                fields_append_pair,
                r#"            __query_pairs.append_pair({name:?}, value);"#
            )?,
            "Option<bool>" => writeln!(
                fields_append_pair,
                r#"            __query_pairs.append_pair({name:?}, if value {{ "true" }} else {{ "false" }});"#
            )?,
            _ => writeln!(
                fields_append_pair,
                r#"            __query_pairs.append_pair({name:?}, &value.to_string());"#
            )?,
        }
        writeln!(fields_append_pair, "        }}")?;
    }

    let watch_append_pair = if is_watch {
    ...
    writeln!(
        ...
        include_str!(concat!(
            ...
            "/templates/query_string_optional.rs"
        )),
        local = local,
        type_name = type_name,
