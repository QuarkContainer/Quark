pub(crate) fn generate(
    ...
    let (visitor_field, visitor_create_field) = if type_generics_type.is_empty() {
        (String::new(), "")
    } else {
        (
            format!("(std::marker::PhantomData{type_generics_type})"),
            "(Default::default())",
        )
    };
    ...
    if resource_metadata.is_some() {
        ...
        writeln!(
            ...
            r#"                            "apiVersion" => Field::Key_api_version,"#
        )?;
        writeln!(
            str_to_field_match_arms,
            r#"                            "kind" => Field::Key_kind,"#
        )?;

        writeln!(
            field_value_match_arms,
            r#"                        Field::Key_api_version => {{"#
        )?;
        writeln!(
            field_value_match_arms,
            r#"                            let value_api_version: String = {local}serde::de::MapAccess::next_value(&mut map)?;"#
        )?;
        writeln!(
            field_value_match_arms,
            r#"                            if value_api_version != <Self::Value as {local}Resource>::API_VERSION {{"#
        )?;
        writeln!(
            field_value_match_arms,
        ...
        writeln!(
            field_value_match_arms,
            r#"                            let value_kind: String = {local}serde::de::MapAccess::next_value(&mut map)?;"#
        )?;
        writeln!(
            field_value_match_arms,
            r#"                            if value_kind != <Self::Value as {local}Resource>::KIND {{"#
        )?;
        writeln!(
            field_value_match_arms,
        ...
        if *is_flattened {
            ...
            match required {
                super::PropertyRequired::Required { is_default } => {
                    writeln!(
                        field_value_defs,
                        r#"                let mut value_{field_name}: Option<{field_type_name}> = None;"#
                    )?;
                ...
                super::PropertyRequired::Optional => {
                    writeln!(
                        field_value_defs,
                        r#"                let mut value_{field_name}: {field_type_name} = None;"#
                    )?;
                ...
                super::PropertyRequired::OptionalDefault => {
                    ...
                    writeln!(
                        field_value_assignment,
                        "                    {field_name}: value_{field_name}.unwrap_or_default(),"
                    )?;
                }
            }
    ...
    let deserialize_type_name = if resource_metadata.is_some() {
        format!("<Self as {local}Resource>::KIND")
    } else {
        format!("{type_name:?}")
    };

    let visitor_expecting_type_name = if resource_metadata.is_some() {
