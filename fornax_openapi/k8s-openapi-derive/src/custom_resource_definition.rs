...
impl super::CustomDerive for CustomResourceDefinition {
    ...
    fn emit(self) -> Result<proc_macro2::TokenStream, syn::Error> {
        ...
        let body_parameter = std::sync::Arc::new(swagger20::Parameter {
        ...
        let name_parameter = std::sync::Arc::new(swagger20::Parameter {
                ...
                (
                    ...
                    Some(std::sync::Arc::new(swagger20::Parameter {
                        ...
                        schema: swagger20::Schema {
                            ...
                            kind: swagger20::SchemaKind::Ty(swagger20::Type::String {
                                format: None,
                            }),
                            kubernetes_group_kind_versions: vec![],
                            list_kind: None,
        ...
        let mut run_state = RunState { writer: vec![] };

        let _ = k8s_openapi_codegen_common::run(
            &spec.definitions,
            &mut spec.operations,
            ...
            &vis,
            if generate_schema {
                k8s_openapi_codegen_common::GenerateSchema::Yes { feature: None }
            } else {
                k8s_openapi_codegen_common::GenerateSchema::No
            },
            None,
            &mut run_state,
        )
        ...
        let out = String::from_utf8(run_state.writer)
            .map_err(|err| format!("#[derive(CustomResourceDefinition)] failed: {err}"))
            .spanning(&tokens)?;
        let result = out
            .parse()
