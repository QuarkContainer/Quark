...
async fn main() -> Result<(), Error> {
    ...
    let out_dir_base = std::sync::Arc::new(
        out_dir_base
            .parent()
            .ok_or("path does not have a parent")?
            .join("src"),
    );

    let requested_versions = if requested_versions.is_empty() {
        supported_version::ALL
            .iter()
            .map(|&version| RequestedVersion {
                version,
    ...
    let tasks: futures_util::stream::FuturesUnordered<_> = requested_versions
        .into_iter()
        .map(
            |RequestedVersion {
                 version,
                ...
                async move {
                    ...
                    let spec_url = overriden_spec_url
                        .as_deref()
                        .unwrap_or_else(|| version.spec_url());
                    logger::TASK_LOCAL_LOGGER
                        .scope(
                            task_local_logger,
...
impl std::str::FromStr for RequestedVersion {
    ...
    fn from_str(spec: &str) -> Result<Self, Error> {
        let (version, overriden_spec_url) =
            spec.split_once(':').map_or((spec, None), |(version, url)| {
                (version, Some(url.to_owned()))
            });

        let &version = supported_version::ALL
            .iter()
            .find(|v| v.name() == version)
            .ok_or_else(|| {
                let mut err =
                    format!("unknown version {version:?} requested, supported versions are ");
...
struct RunState<'a> {
    ...
    parent_mod_rs_file_and_mod_name: Option<(
        <Self as k8s_openapi_codegen_common::RunState>::Writer,
...
impl k8s_openapi_codegen_common::RunState for RunState<'_> {
    ...
    fn make_writer(
        ...
        for part in parts.iter().skip(1).rev().skip(1).rev() {
            ...
            if !current.is_dir() {
                log::trace!(
                    "    Subdirectory does not exist. Creating mod.rs with a reference to it..."
                );

                let current_mod_rs_path = current.with_file_name("mod.rs");
                let append_newline = current_mod_rs_path.exists();
                let mut parent_mod_rs = std::io::BufWriter::new(
                    std::fs::OpenOptions::new()
                        .append(true)
                        .create(true)
                        .open(current_mod_rs_path)?,
                );
                if append_newline {
    ...
    fn handle_operation_types(
        ...
        match (operation_optional_parameters_name, operation_result_name) {
            (Some(operation_optional_parameters_name), Some(operation_result_name)) => writeln!(
                parent_mod_rs,
            ...
            (None, Some(operation_result_name)) => writeln!(
                parent_mod_rs,
