...
impl log::Log for Logger {
    fn enabled(&self, metadata: &log::Metadata<'_>) -> bool {
        TASK_LOCAL_LOGGER
            .try_with(|logger| logger.enabled(metadata))
            .unwrap_or_else(|_| {
    ...
    fn log(&self, record: &log::Record<'_>) {
        TASK_LOCAL_LOGGER
            .try_with(|logger| logger.log(record))
            .unwrap_or_else(|_| {
                THREAD_LOCAL_LOGGER.with(|thread_local_logger| {
                    if let Some(logger) = thread_local_logger.borrow().as_ref() {
                        logger.log(record);
                    }
                })
            });
...
pub(crate) fn register_thread_local_logger(logger: env_logger::Logger) {
    THREAD_LOCAL_LOGGER
        .with(|thread_local_logger| *thread_local_logger.borrow_mut() = Some(logger));
}
